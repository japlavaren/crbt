...
class Bot:
    def __init__(self, symbol: str, min_buy_price: Decimal, max_buy_price: Decimal, step_price: Decimal,
                 min_profit: Decimal, kline_margin: Decimal, trade_amount: Decimal, api: Api) -> None:
        ...
        self._positions: Positions = Positions(min_buy_price, max_buy_price, step_price)
    ...
    def statistics(self) -> Dict[str, Any]:
        ...
        return dict(
            ...
            opened_revenue=sum(position.trade.calculate_revenue(current_price) for position in bought_positions),
    ...
    def _investment(self) -> Decimal:
        return sum(position.trade.bought_amount for position in self._positions.bought_positions)
    ...
    def _create_sell_orders(self) -> None:
        for position in self._positions._positions:
            if position.trade is not None and position.trade.status == Trade.STATUS_BOUGHT:
